...
exports.filtreTexte = filtreTexte;
...
var token = "M. Mime";
var sanitizedToken = token.replace(/\s/g, ''); // Supprimer les espaces HTML du jeton
...
$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function success(data) {
      dataPokemon = data;
      var inputSearch = document.querySelector("#searchPokemon");
  ...
  function renderCards(pokemonData) {
    ...
    pokemonData.forEach(function (data) {
      var card = (0, _function.createCard)(data.name, data.apiTypes, data.image, data.id);
      main.append(card);
      $(card).click(function (e) {
        ...
        var cardId = Number(this.id);
        var selectedPokemon = pokemonData.find(function (pokemon) {
          return pokemon.id === cardId;
        });
        var cardDetails = (0, _function.cardPokemon)(selectedPokemon.name, selectedPokemon.apiTypes, selectedPokemon.image, selectedPokemon.stats.HP, selectedPokemon.stats.attack, selectedPokemon.stats.defense, selectedPokemon.apiEvolutions);
  ...
  $("form").on("submit", function (event) {
    ...
    (0, _function.classSwitch)($("main"), "in-active", "active");
    (0, _function.classSwitch)($(".pokemon"), "active", "in-active");
    var searchInputPokemon = (0, _function.filtreTexte)(dataPokemon, $("#searchPokemon").val());
    if (searchInputPokemon.length > 0) {
      var selectedPokemon = searchInputPokemon[0];
      var cardDetails = (0, _function.cardPokemon)(selectedPokemon.name, selectedPokemon.apiTypes, selectedPokemon.image, selectedPokemon.stats.HP, selectedPokemon.stats.attack, selectedPokemon.stats.defense, selectedPokemon.apiEvolutions);
      container.append(cardDetails);
      $("#close").click(function () {
        $(".pokemon").empty();
